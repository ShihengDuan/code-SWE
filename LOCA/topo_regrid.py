# Get static topo for LOCA grid points. Used for projection. 
# Instead of nearest neighbot, use average of surrounding grid points. 
# 30m dem grid spacing: 
# LOCA grid spacing: 0.0625 degree
import xarray as xa
import xesmf as xe
import numpy as np
import glob
...
for mountain in mountains:
    ...
    for i, lat in tqdm(enumerate(lats)):
        for j, lon in enumerate(lons):
            ...
            ele_station = ele.sel(y=slice(lat+0.03, lat-0.03), x=slice(lon-0.03, lon+0.03)).mean(skipna=True).data
            elevation_30m[i, j] = ele_station
            
            # dah_station = dah.sel(y=lat, x=lon, method='nearest').data[0]
            dah_station = dah.sel(y=slice(lat+0.03, lat-0.03), x=slice(lon-0.03, lon+0.03)).mean(skipna=True).data
            # print(dah_station)
            ...
            trasp_station = trasp.sel(y=slice(lat+0.03, lat-0.03), x=slice(lon-0.03, lon+0.03)).mean(skipna=True).data
    ...
    static_data.to_netcdf(mountain+'_topo_file_30m_ave.nc')
