...
embedding = 32
model = args['model']
scenario = '85'  # 'hist' or '85
attributions = ['longitude', 'latitude', 'elevation_prism', 'dah', 'trasp']
forcings = {'pr': model + '-' + scenario + '-pr.nc', 'rave': model + '-' + scenario + '-rave.nc',
            'sph': model + '-' + scenario + '-sph.nc', 'srad': model + '-' + scenario + '-srad.nc',
            'tmmn': model + '-' + scenario + '-tmmn.nc', 'tmmx': model + '-' + scenario + '-tmmx.nc',
            'vs': model + '-' + scenario + '-vs.nc'}
...
def inference(lat_id, device=device):
    ds = CO_LOCADataset(forcings=forcings, lat=lat_id, attributions=attributions, topo_file=topo_file,
                        window_size=180, scenario=scenario)
    if model_type == 'LSTM':
...
if __name__ == '__main__':
    ...
    np.save(path + model + '-' + scenario + '_' + str(ens), prediction)
