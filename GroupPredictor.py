...
def train(model, ds, lr, device=torch.device('cuda:0'), writer=None):
    ...
    for epoch in tqdm(range(50)):
        ...
        if epoch == 47:
            model2 = model
        if ((epoch+1)%2==0):
            torch.save(model.state_dict(), path +  'model_ens_' + str(e)+'e'+str(epoch))
    return model, model2
...
target = ['SWE']
permute_id = [2, 3, 4, 5, 6, 7, 8] # precip + ave_temperature
...
for station_id in range(581):
    ds = gridMETDatasetStationAveTemp(forcings=forcings, attributions=attributions, target=target, window_size=WINDOW_SIZE,
...
for e in range(5, 10):  # ens number
    ...
    for station_id in tqdm(range(581), desc='test_ds'):
        ds = gridMETDatasetStationAveTemp(forcings=forcings, attributions=attributions, target=target,
